...
from diffusers import DDIMScheduler, StableDiffusionPipeline
from util import save_video, seed_everything
from tokenflow_utils import register_extended_attention_pnp, set_tokenflow
...
class TemporalConsistency(nn.Module):
    def __init__(self, config):
        ...
        self.frames, self.latents, self.eps = self.load_latents()

    def load_latents(self):
        latents_path = os.path.join(self.latents_path, "latents")
        noisy_latents = torch.load(glob.glob(f"{latents_path}/noisy_latents_*.pt")[-1]).to(self.device)
        clean_latents = torch.load(glob.glob(f"{latents_path}/latents.pt")[-1]).to(self.device)
        return None, clean_latents, noisy_latents

    @torch.no_grad()
    def apply_temporal_consistency(self, x):
        set_tokenflow(self.unet)  # Apply TokenFlow consistency adjustments
        for t in tqdm(self.scheduler.timesteps, desc="Applying Temporal Consistency"):
            x = self.scheduler.step(self.unet(x, t)["sample"], t, x)["prev_sample"]
        return x

    def process_video(self):
        output_path = os.path.join(self.config["output_path"], "consistent_video.mp4")
        refined_latents = self.apply_temporal_consistency(self.latents)
        refined_video = self.decode_latents(refined_latents)
        save_video(refined_video, output_path, fps=20)
...
if __name__ == '__main__':
    ...
    config["output_path"] = os.path.join(config["output_path"], "temporal_consistency_only")
    os.makedirs(config["output_path"], exist_ok=True)
